...
import mx.com.segurossura.grouplife.openapi.model.FindDocumentResponseDto;
...
public class PrintQuoteIntegrationTest extends BaseIT {

    private static final String BASE_PATH = "/print-quote/";
    ...
    private WebClient muiJasperWebClient;
    ...
    void setup() {
        ...
        when(this.muiJasperWebClient.post()).thenReturn(this.requestBodyUriSpec);
    ...
    void test_getReportQuotation_success() {
        ...
        final JasperDataResponseDto jasperDataResponseDto = createJasperDataResponseDto();
        when(this.responseSpec.bodyToMono(JasperDataResponseDto.class)).thenReturn(Mono.just(jasperDataResponseDto));

        final ExecutiveInfoDataResponseDto executiveInfoDataResponseDto = createExecutiveInfoDataResponseDto();
        when(this.responseSpec.bodyToMono(ExecutiveInfoDataResponseDto.class)).thenReturn(Mono.just(executiveInfoDataResponseDto));

        final StorageResponseDto storageResponseDto = createStorageResponseDto();
        when(this.responseSpec.bodyToMono(StorageResponseDto.class)).thenReturn(Mono.just(storageResponseDto));
        ...
        final FindDocumentResponseDto actualResponseJasper =
                this.webTestClient.get()
                        .uri(BASE_PATH + "19925")
                        .headers(headers -> headers.setBasicAuth(USERNAME, PASSWORD))
                        .accept(MediaType.APPLICATION_JSON)
                        ...
                        .expectStatus().isOk()
                        .expectHeader().contentType(MediaType.APPLICATION_JSON)
                        .expectBody(FindDocumentResponseDto.class)
                        .returnResult()
                        .getResponseBody();

        assertNotNull(actualResponseJasper);
    }

    @Test
    void test_getReportQuotation_volunteer_success() {
        ...
        final GetCoveragesByModality200ResponseDto actualResponse =
                this.webTestClient.get()
                        .uri(BASE_PATH_CATALOG + "?modality=VOLUNTARIA")
                        .headers(headers -> headers.setBasicAuth(USERNAME, PASSWORD))
                        .accept(MediaType.APPLICATION_JSON)
                        .exchange()
                        .expectStatus().isOk()
    ...
    private void createFolio() {
        ...
        FolioNumber folioNumber = new FolioNumber("19925");
        ...
        folioRecord.setSami(new BigDecimal("5000000"));
    ...
    private PlanDataResponseDto.PlanResponseListDto createPlanDataResponseDtoGtw() {
        ...
        PeriodicResponseDto periodicResponseDto = PeriodicResponseDto.builder()
                ...
                .firstAnnualReceipt(new BigDecimal("4984.5528"))
                .firstSemiannualReceipt(new BigDecimal("4984.5528"))
                .firstQuarterlyReceipt(new BigDecimal("4984.5528"))
                .firstMonthlyReceipt(new BigDecimal("4984.5528"))
                .subsequentAnnualReceipt(new BigDecimal("4984.5528"))
                .subsequentSemiannualReceipt(new BigDecimal("4984.5528"))
                .subsequentQuarterlyReceipt(new BigDecimal("4984.5528"))
                .subsequentMonthlyReceipt(new BigDecimal("4984.5528"))
                .build();
    ...
    private InsuranceDataDto createInsuranceDataDto() {
        ...
        final CoverageCatalogDto coverageCatalogList2 = new CoverageCatalogDto("VOLUNTARIA", "Modalidad Voluntaria", insuredDto, ageDto,
                List.of(coverageDetailDto), 10, null, null, null, null, null, null, null, null
        );

        final CatalogDataDto catalogDataDto = new CatalogDataDto("VIDA_GRUPO", "Catálogo de Coberturas Vida Grupo", List.of(coverageCatalogList, coverageCatalogList2));

        return new InsuranceDataDto(List.of(catalogDataDto));
    }

    private JasperDataResponseDto createJasperDataResponseDto() {
        JasperDataResponseDto.JasperContentResponseDto jasperContentResponseDto = JasperDataResponseDto.JasperContentResponseDto.builder()
                .fileName("Cotizacion_vida_grupo_123.pdf")
                .fileText("tyvbrtyvtrbvrtybtybutyubbtybutbyuby")
                .build();

        return new JasperDataResponseDto(jasperContentResponseDto);
    }

    private ExecutiveInfoDataResponseDto createExecutiveInfoDataResponseDto() {
        ExecutiveInfoDataResponseDto.ExecutiveInfo executiveInfo = ExecutiveInfoDataResponseDto.ExecutiveInfo.builder()
                .insuranceAgentKey("000001")
                .nameAgent("CLIENTE ESPECIAL3291 1")
                .pointSaleKey("100001")
                .pointSaleDescription("PUNTO DE VENTA DEMO PARA EASY-ALEA")
                .productKey("902")
                .subgroupKey("0547000001")
                .subgroupDescription("VIDA GRUPO DIGITAL TRADICIONAL")
                .profileTariffKey("9020000001")
                .profileTariffDescription("VIDA GRUPO MODALIDAD TRADICIONAL")
                .officeKey("1")
                .officeDescription("OFICINA MEXICO, D.F.")
                .promoterKey("794")
                .promoterName("BEATRIZ  MENA MARTINEZ (METROPOLITANA)")
                .banck("No encontrado")
                .executiveKey("153")
                .executiveName("BEATRIZ  MENA MARTINEZ")
                .subaddressKey("012")
                .subaddressName("SUBDIRECCION CORREDORES PERSONAS FISICAS")
                .managerKey("126")
                .managerName("EDER VÁZQUEZ LEÓN")
                .regionalKey("019")
                .desciptionRegional("CDMX")
                .build();

        return new ExecutiveInfoDataResponseDto(List.of(executiveInfo));
    }

    private StorageResponseDto createStorageResponseDto() {
        final StorageResponseDto.Files files = new StorageResponseDto.Files(
                "nombrepdf",
                "http://rutapdf.pdf"
        );

        final StorageResponseDto.StorageResponseDataDto storageResponseDataDto = new StorageResponseDto.StorageResponseDataDto(
                "subido exitosamente",
                List.of(files)
        );

        return new StorageResponseDto(storageResponseDataDto);
    }
    ...
    private void createFolioVolunteer() {
        ...
        FolioNumber folioNumber = new FolioNumber("19926");
        QuotationDetails quotationDetails = new QuotationDetails(0.2, 0.01, 0.2,
                0.01, LocalDate.parse("2024-11-09"), LocalDate.parse("2025-11-09"), "C", "DETALLADA");
        CatalogItem catalogItem = new CatalogItem("001", "GIRO");
        Company company = new Company("acme", catalogItem, 30, 100, 23, new BigDecimal(49), new BigDecimal(25));
        ...
        folioRecord.setStatus("Abierto");
        folioRecord.setQuotationDetails(quotationDetails);
        folioRecord.setSami(new BigDecimal(10000));
