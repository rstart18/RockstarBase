...
import mx.com.segurossura.grouplife.infrastructure.adapter.gateway.StorageGateway;
...
public class StorageGatewayTest {
    ...
    @Test
    void getFileUrlStorage_data_null() {

        final StorageResponseDto storageResponseDto = new StorageResponseDto(null);
        final StorageRequest storageRequest = new StorageRequest(null);

        when(responseSpec.bodyToMono(StorageResponseDto.class)).thenReturn(Mono.just(storageResponseDto));

        StepVerifier.create(storageGateway.getFileUrlStorage(storageRequest))
                .expectErrorMatches(throwable -> throwable instanceof GatewayException &&
                        ((GatewayException) throwable).getCode().contains("VG-GTW-NULL-STORAGE"))
                .verify();
    }
